...
function FriendRecommendPage() {
	...
	useEffect(() => {
		async function fetchData() {
			const resFriend = await friendService.handleFriendRecommendService(userId);
	...
	return (
		<div className={cx("container",currentTheme === THEMES.DARK && THEMES.DARK)}>
			...
			{friend.length > 0 && friend.map((data) => <CompCard data={data} key={Math.random()} recommend/>)}
